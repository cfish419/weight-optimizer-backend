...
async def calculate_loading_plan(request: OptimizationRequest, db: Session = Depends(get_db)):
    ...
    baggage_items = db.query(Baggage).filter(Baggage.flight_id == request.flight_id).all()
    if not baggage_items:
        raise HTTPException(status_code=400, detail="No baggage items found for this flight")
    ...
    for idx, bag in enumerate(baggage_items):
        ...
        instructions.append(f"Load bag {bag.tag_number} in {pos['zone']} at position {idx + 1}")
