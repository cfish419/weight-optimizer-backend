...
class TestBaggageFlow(unittest.TestCase):
    ...
    def test_gate_check_scenario(self):
        ...
        gate_item = next(
            item for item in flight_baggage if item["baggage_id"] == gate_baggage_id
        )
    ...
    def test_special_item_handling(self):
        ...
        self.assertEqual(
            golf_item["loading_priority"], 2
        )  # High priority for special items
    ...
    def test_offline_sync_flow(self):
        ...
        sync_result = self.offline_service.sync_offline_changes(
            agent_id, offline_changes
        )
