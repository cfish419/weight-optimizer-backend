...
async def handle_passenger_noshow(
    request: PassengerNoShowRequest, scenario_service: ScenarioService = Depends()
):
...
async def handle_gate_check(
    request: GateCheckRequest, scenario_service: ScenarioService = Depends()
):
...
async def handle_aircraft_swap(
    request: AircraftSwapRequest, scenario_service: ScenarioService = Depends()
):
...
async def handle_emergency_reduction(
    request: EmergencyReductionRequest, scenario_service: ScenarioService = Depends()
):
...
async def handle_weather_update(
    request: WeatherUpdateRequest, weather_service: WeatherService = Depends()
):
    ...
    try:
        ...
        arrival_weather = await weather_service.get_weather_data(
        ...
        performance_limits = weather_service.get_performance_limitations(
            departure_weather
        )

        # Calculate fuel adjustments
        base_fuel = 18000  # kg - would come from flight plan
        adjusted_fuel = weather_service.calculate_fuel_adjustment(
            departure_weather, base_fuel
        )
...
async def get_active_scenarios(
    flight_id: str, scenario_service: ScenarioService = Depends()
):
