...
class OfflineRepository:
    ...
    def mark_change_applied(self, change_id: str) -> bool:
        ...
        try:
            if change_id in self.queued_changes:
                ...
                self.queued_changes[change_id]["applied_at"] = datetime.utcnow().isoformat()
                return True
            return False
        except Exception:
            return False
    ...
    def mark_change_failed(self, change_id: str, error_message: str) -> bool:
        ...
        try:
            if change_id in self.queued_changes:
                ...
                self.queued_changes[change_id]["failed_at"] = datetime.utcnow().isoformat()
                return True
            return False
        except Exception:
            return False
    ...
    def mark_change_invalid(self, change_id: str) -> bool:
        ...
        try:
            if change_id in self.queued_changes:
                ...
                self.queued_changes[change_id]["invalidated_at"] = datetime.utcnow().isoformat()
                return True
            return False
        except Exception:
            return False
    ...
    def increment_retry_count(self, change_id: str) -> bool:
        ...
        try:
            if change_id in self.queued_changes:
                ...
                self.queued_changes[change_id]["last_retry"] = datetime.utcnow().isoformat()
                return True
            return False
        except Exception:
            return False
    
    def get_conflicting_changes(self, resource_id: str, after_timestamp: datetime) -> List[Dict[str, Any]]:
        ...
        for change in self.queued_changes.values():
            ...
            if (change_data.get("baggage_id") == resource_id and
    ...
    def save_offline_snapshot(self, snapshot_id: str, snapshot_data: Dict[str, Any]) -> bool:
